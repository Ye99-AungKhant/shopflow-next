    ...
    <div className="space-y-6">
        ...
        {kpiCards.map((card) => {
          ...
          return (
            ...
            >
              ...
              <div className={`mt-4 flex items-center gap-2 text-sm ${trendColor}`}>
      ...
      {!isSupabaseConfigured && (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.25)]">
          Please configure your Supabase URL and Anon Key in the environment variables to load dashboard data.
        </div>
      )}
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="rounded-3xl bg-white p-6 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.25)] xl:col-span-2">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">A smooth view of daily sales momentum across the week.</p>
            </div>
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.revenueData} margin={{ top: 10, right: 8, left: -16, bottom: 0 }}>
                <defs>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.25)]">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-slate-500">Highest lifetime value customers from your order history.</p>
          </div>

          <div className="space-y-4">
            {data.topCustomers.length === 0 && (
              <p className="text-sm text-slate-500">No customer purchases yet.</p>
            )}
              ...
              <div key={customer.name} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Loyal repeat customer</p>
                  </div>
                </div>
                <p className="text-sm font-semibold text-slate-900">{customer.ltv}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.25)]">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-slate-500">Best-performing items by units sold and revenue generated.</p>
        </div>
