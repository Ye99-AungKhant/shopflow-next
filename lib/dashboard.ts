...
import { isSupabaseConfigured, supabase, type Customer, type InventoryItem, type Order } from './supabase';
...
export async function fetchDashboardData(): Promise<DashboardData> {
  ...
  const [ordersResult, customersResult, inventoryResult] = await Promise.all([
    ...
    supabase.from('inventory').select('id, name, sku, stock_quantity, price, created_at'),
  ]);
  ...
  const currentRevenue = currentOrders.reduce((sum, order) => sum + Number(order.total_price ?? 0), 0);
  ...
  for (const order of orders) {
    ...
    if (revenueLookup.has(key)) {
      revenueLookup.set(key, (revenueLookup.get(key) ?? 0) + Number(order.total_price ?? 0));
    }
  }

  const revenueData = Array.from(revenueLookup.entries()).map(([date, revenue]) => ({
  ...
  const inventoryNameById = new Map(inventory.map((item) => [item.id, item.name]));
  ...
  for (const order of orders) {
    for (const item of order.order_items ?? []) {
      const productName = item.inventory_id
        ...
        : item.name;
      const existing = productStats.get(productName) ?? { units: 0, revenue: 0 };
